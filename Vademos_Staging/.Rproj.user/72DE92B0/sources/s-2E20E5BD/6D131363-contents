...
colnames(data)[26] <- 'lat'
colnames(data)[27] <- 'lon'
#data$TotalOutbreaks[data$TotalOutbreaks == 0] <- NA
...
#ivory coast
data$TotalOutbreaks[1151:1180][is.na(data$TotalOutbreaks[1151:1180])] <- 0
#gambia
data$TotalOutbreaks[931:960][is.na(data$TotalOutbreaks[931:960])] <- 0
#kenya
data$TotalOutbreaks[1201:1230]<- 102
#burundi
data$TotalOutbreaks[541:570]<- 27
#djbouiti
data$TotalOutbreaks[771:790]<- 0
#uganda
data$TotalOutbreaks[2065:2094]<- 15
#zambia
data$TotalOutbreaks[2195:2224]<- 0
#cameron
data$TotalOutbreaks[571:600]<- 15
#guinea
data$TotalOutbreaks[1021:1050]<- 35
#nepal
data$TotalOutbreaks[1481:1510]<- 45
#UAE
data$TotalOutbreaks[2095:2144]<- 10
#afghanistan
data$TotalOutbreaks[1:30]<- 170

#angola
data$TotalOutbreaks[291:320]<- 0
#south sudan
data$TotalOutbreaks[1823:2383]<- 0

#malawi
data$TotalOutbreaks[1341:1370]<- 1

#saudi arabia
data$TotalOutbreaks[1713:1732]<- 1

#bahrain
data$TotalOutbreaks[381:400]<- 15

#sri lanka
data$TotalOutbreaks[1839:1868]<- 125

#guinea bissau
data$TotalOutbreaks[1051:1080]<- 9


#equatorial guinea
data$TotalOutbreaks[821:850]<- 0


#middle east 
data$TotalOutbreaks[141:170]<- 450

#all africa
data$TotalOutbreaks[21:50]<-500

#
#data$TotalOutbreaks[1181:2294][is.na(data$TotalOutbreaks[1181:2294])] <- 0




data$PCP <- as.numeric(data$PCP)
data$PCP[is.na(data$PCP)] <- 1
data$Subregion[which(data$Country_name == "All Asia")] <- 'All Asia Subregions'
data$PCP[which(data$PCP == 0)] <- 1

data$PopPropByRegion[which(data$Country_name == "Turkey (Anatolia)")] <- 0.07
data$PropDairy[which(data$Country_name == "Equatorial Guinea")] <- 0.05
data$PropBeef[which(data$Country_name == "Equatorial Guinea")] <- 0.35
delphi <- read.csv('delphi-round1.csv')
data$Pop <- trunc(data$Pop)
...
get.outbreaks <- function(df_train, df_test, df_pred){
  poisson.model <- glm(TotalOutbreaks ~ PCP + TotalPop, #Year+PCP, #+TotalPop, #here
                       df_train,
                       na.action=na.exclude,
                       family=quasipoisson(link="log"))#here quasi to poisson
  ...
  ON_predicted <- round((as.integer(predict(poisson.model,
                                     df_pred,
                                     na.action=na.exclude,
                                     type="response")))/2)
...
get_inputtable <- function(df, countrycode, year_to_predict, subnational=NA){
  ...
  # New dataframe with predicted pop column added
  df_pred <- get.df.with.pop.predicted(df.c, countrycode, year_to_split, year_to_predict, subnational)
...
get_reducedtable <- function(df_inputtable){
  ...
  df_lr <- df_inputtable %>%
    filter(Year > 2021, Species == "LR") %>%
    select(c("Year", "PCP", "Pop_predicted"))
  
  df_sr <- df_inputtable %>%
    filter(Year > 2021, Species == "SR") %>%
    select(c("Year", "Pop_predicted"))
  
  df_p <- df_inputtable %>%
    filter(Year > 2021, Species == "P") %>%
...
run_mc <- function(df_inputtable, year_to_predict,
  ...
  Pop_VA_LR <<- 100 * LSD_LR #changed to 10km
  Pop_VA_SR <<- VA * LSD_SR
  Pop_VA_P <<- VA * LSD_P
