...
# 1 - Data & Source files
###################################################
#data <- read.csv('data_Mar2022 18 03 22.csv', sep = ",")
data <- read.csv('newdata.csv', sep = ",")
colnames(data)[10] <- "Agricultural.land"
data$TotalOutbreaks[is.na(data$TotalOutbreaks)] <- 0
data$PopGrowth[is.na(data$PopGrowth)] <- 0
...
get.growthrate.predicted <- function(df){
  pct_changes <- df %>%
    split(df$Species) %>%
    sapply(function(x) {-diff(x$Pop_predicted)/x$Pop_predicted[-1]})
  ...
  df2 <- data.frame()
  for (s in unique(df$Species)){
    filtered_by_pop <- filter(df, Species == s & Year %in% years)
    filtered_by_pop$PopGrowth <- pct_changes[, s]
    df2 <- rbind(df2, filtered_by_pop)
  } # for loop end
  return(df2)
} # function end
...
get.df.delphi <- function(delphi, pcp){
  ...
  if (pcp %in% c(1,2,3,4)){
    ...
    df_delphi <- data.frame(list(q2_, q3, q4))
    ...
    df_delphi <- data.frame(list(q2, q3, q4))
  } else{
...
get_inputtable <- function(df, countrycode, year_to_predict, subnational=NA){
  ...
  #df_prev <- subset(data, Year <2020 ) #here debug adding prevop as real pop  
  df_prev <- df.c[df.c$Year < min(df_pred$Year), ] #subsituted by the above
  
  #TotalPop<- aggregate(prevpops$Pop, by=list(Category=prevpops$Country_code, prevpops$Year),  FUN=sum, na.rm=TRUE) # here debug replaced dfprev by actual prevop
  TotalPop <- aggregate(df_prev$Pop, by=list(Category=df_prev$Country_code, df_prev$Year), FUN=sum, na.rm=TRUE, na.action=na.omit) # use real pop values
...
get_results <- function(df, countrycode, year_to_predict,
  ...
  for (year in c(2020:year_to_predict-1)){
    ...
    vaccine_estimatesS_ <- results[[5]] # Rob (16/02/22)
      ...
      vaccine_estimatesS_ <- results[[5]] #Rob (16/02/22)
      df_result <- bind_rows(df_result, vaccine_estimates_)
      df_resultS <- bind_rows(df_resultS, vaccine_estimatesS_) #Rob (16/02/22)
    }
  }
