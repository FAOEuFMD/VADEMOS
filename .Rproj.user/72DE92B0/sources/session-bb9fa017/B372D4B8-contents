...
######################Results#######################################
  ...
  get_results <- function() {
    ...
    for (i in seq_len(nrow(forecast_data))) {
      ...
      pcp_2024 <- pcp_filtered_country[pcp_filtered_country$Year == 2024, ]  # Filter for the year 2024
      
      # Print the filtered PCP data for debugging
      print("Step 3: PCP Data for 2024")
      
      
      # Pull the numeric PCP stage as numeric values
      pcp_stage <- as.numeric(pcp_2024$`PCP-FMD Stage Numeric`)  # Directly access the numeric stage
      ...
      results <- rbind(results, data.frame(Country = country,
                                           ...
                                           Youngstock_Coverage = round(youngstock_vaccine_requirement,2),  # Actual youngstock to vaccinate
                                           Adult_Coverage = round(adultstock_vaccine_requirement,2),       # Actual adults to vaccinate
                                           VaccineRequirement = total_vaccine_requirement))
    }
  ...
  observeEvent(input$mapbutton, {
    ...
    merged <- merged %>% mutate(Vaccine_Requirement = round((Density / 100) * VaccineRequirement, 2))
    ...
    sf_data <- merge(sf_data, merged, by = "ADM1_Name", all.x = TRUE)
